...
def calc_f1(p_anss, anss, predictions=None, evidences=None):
    total_count = 0
    answer_f1 = 0
    if predictions is None:
        for p_a, a in zip(p_anss, anss):
            total_count += 1
            temp_answer_f1 = 1 if p_a == a else 0
            answer_f1 += temp_answer_f1

        answer_f1_score = 100.0 * answer_f1 / total_count
        return answer_f1_score

    evidence_f1 = 0
    all_f1 = 0
    for p_a, a, prediction, evidence in zip(p_anss, anss, predictions, evidences):
        total_count += 1
        # print(p_a, a)
        temp_answer_f1 = 1 if p_a == a else 0
        temp_evidence_f1 = calc_f1_score(evidence, prediction)
        answer_f1 += temp_answer_f1
        evidence_f1 += temp_evidence_f1
        all_f1 += temp_answer_f1 * temp_evidence_f1
    all_f1_score = 100.0 * all_f1 / total_count
    ...
    return all_f1_score, answer_f1_score, evidence_f1_score
...
def calc_f1_solo(ans, prediction):
    pred_toks = normalize_answer(mixed_segmentation(prediction))
    gold_toks = normalize_answer(mixed_segmentation(ans))
    common = collections.Counter(gold_toks) & collections.Counter(pred_toks)
    num_same = sum(common.values())
...
if __name__ == '__main__':
    ...
    # print(calc_f1_score(['这是证据1', '这是证据2'], '证据1'))
